...
-- Only one physical monitor is present, so these have no valid target:
...
-- On-screen keyboard manual toggle (squeekboard usually pops up on its own,
-- this is a fallback for apps/fields that don't trigger it, e.g. XWayland)
hl.bind(mainMod .. " + K", hl.dsp.exec_cmd("~/.config/hypr/scripts/toggle-osk.sh"))

-- Power button -> log out (back to greetd session picker); ROG/Armoury Crate key (sends XF86Launch3) -> actual power off
hl.bind("XF86PowerOff",  hl.dsp.exec_cmd(noctCall .. "session logout"), { locked = true })
hl.bind("XF86Launch3",   hl.dsp.exec_cmd("systemctl poweroff"),         { locked = true })

-- Display mirror/extend (Fn+F9 sends raw SUPER+P on this keyboard's firmware)
hl.bind(mainMod .. " + P", hl.dsp.exec_cmd("~/.config/hypr/scripts/toggle-display-mirror.sh"))
...
-- Minimize to dock (own special workspace, kept separate from the scratchpad above).
-- Minimized apps stay listed in the Noctalia dock (show_running) so you can restore
-- them from there; SUPER+SHIFT+M is a keyboard fallback that reveals/hides them.
-- Moving the focused window into a special workspace leaves that workspace shown
-- (it follows the focused window), so the toggle_special right after is required
-- to actually hide it rather than just relocate it in place.
